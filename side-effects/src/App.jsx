...
function App() {
	...
	function handleSelectPlace(id) {
		...
		if (sortedIds.indexOf(id) === -1) {
			localStorage.setItem('selectedPlaces', JSON.stringify([id, ...sortedIds]));
		}
	}
	...
	const handleRemovePlace = useCallback(() => {
		...
		setModalIsOpen(false);
		const sortedIds = JSON.parse(localStorage.getItem('selectedPlaces')) || [];
		localStorage.setItem('selectedPlaces', JSON.stringify(sortedIds.filter((id) => id !== selectedPlace.id)));
	}, []);
